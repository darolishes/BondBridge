      ...
      try {
        ...
        if (stat.isDirectory()) {
          ...
          if (!skipDirs.includes(file)) {
            ...
            console.log(
              `Skipping directory: ${filePath} (for scan efficiency)`
            );
          }
        } else {
        ...
        console.error(
          `Error processing ${path.join(dir, file)}: ${error.message}`
        );
      }
    });
...
function generateRooignore(directory, tokenThreshold) {
  try {
    ...
    console.log(
      `Skipping ${skipDirs.length} directories to improve scan performance`
    );
    ...
    if (existingRooignore.exists) {
      console.log(
        `Found existing .rooignore with ${existingRooignore.patterns.length} patterns`
      );
    }
    ...
    for (const file of files) {
      ...
      if (processedCount % 100 === 0 || processedCount === files.length) {
        process.stdout.write(
          `\rAnalyzing files: ${processedCount}/${files.length} (${Math.floor(
      ...
      if (tokens > tokenThreshold) {
        ...
        process.stdout.write('\n');
        console.log(
          `Large file found: ${relativePath} (approximately ${tokens} tokens)`
        );
      }
    }
    ...
    const newLargeFiles = largeFiles.filter(
      (file) => !existingRooignore.patterns.includes(file)
    );

    if (newLargeFiles.length > 0) {
      console.log(
        `Found ${newLargeFiles.length} new large files to be added to .rooignore`
      );
    } else if (largeFiles.length > 0) {
      console.log(
        `All ${largeFiles.length} large files are already in .rooignore`
      );
    }
    ...
    if (!existingRooignore.exists) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          line.includes('# Files over') &&
          line.includes('tokens (LLM context overflow prevention)')
        ) {
      ...
      if (!insertedLargeFiles) {
        ...
        let insertPos = 0;
        while (
          insertPos < updatedLines.length &&
          updatedLines[insertPos].startsWith('#')
        ) {
          insertPos++;
        }
    ...
    console.log(
      `Detected ${largeFiles.length} large files (${newLargeFiles.length} newly added)`
    );
  } catch (error) {
