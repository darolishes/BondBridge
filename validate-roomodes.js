...
function validateRoomodes(filePath, fixMode = false) {
  ...
  try {
    ...
    console.log(
      `   Tip: Check for missing commas, unbalanced brackets, or quotes`
    );
    result.isValid = false;
  ...
  if (!parsedContent.customModes && !parsedContent.modes) {
    ...
    console.log(
      `   Tip: File should have structure: { "customModes": [ ... ] }`
    );
    hasErrors = true;
    result.isValid = false;
  ...
  modesData.forEach((mode, index) => {
    console.log(
      `Checking mode #${index + 1}${mode.name ? ` (${mode.name})` : ''}:`
    );
    let modeValid = true;
    ...
    if (!mode.name) {
      ...
      if (fixMode) {
        const name = mode.slug
          ? mode.slug.charAt(0).toUpperCase() +
            mode.slug.slice(1).replace(/-./g, (x) => x[1].toUpperCase())
          : `Mode ${index + 1}`;
    ...
    if (!mode.roleDefinition) {
      ...
      if (fixMode) {
        ...
        fixedContent.customModes[
          index
        ].roleDefinition = `You are Roo, a ${name.toLowerCase()} assistant.`;
      ...
      if (fixMode) {
        fixedContent.customModes[index].roleDefinition = String(
          mode.roleDefinition
        );
      ...
      if (!mode.roleDefinition.includes('You are Roo')) {
        console.log(
          `  ⚠️  WARNING: roleDefinition should typically start with "You are Roo"`
        );

        if (
          fixMode &&
          !fixedContent.customModes[index].roleDefinition.startsWith(
            'You are Roo'
          )
        ) {
          fixedContent.customModes[index].roleDefinition = `You are Roo, ${
            fixedContent.customModes[index].roleDefinition
              .charAt(0)
              .toLowerCase() +
            fixedContent.customModes[index].roleDefinition.slice(1)
          }`;
          console.log(
            `    Auto-fix: Prepended "You are Roo" to roleDefinition`
          );
        }
      }
    }
    ...
    if (!mode.groups) {
      ...
      if (fixMode) {
        fixedContent.customModes[index].groups = ['read'];
        console.log(
          `    Auto-fix: Replaced invalid groups with default "read" group`
        );
      }
    } else {
      const { errors, warnings, fixedGroups } = validateGroups(
        mode.groups,
        fixMode
      );
  ...
  if (hasErrors) {
    ...
    if (fixMode && parsedContent.modes && !parsedContent.customModes) {
      fixedContent.customModes = JSON.parse(
        JSON.stringify(parsedContent.modes)
      );
      result.fixedContent = JSON.stringify(fixedContent, null, 2);
...
function validateGroups(groups, fixMode = false) {
  ...
  groups.forEach((group, index) => {
    ...
    if (typeof group === 'string') {
      ...
      if (!VALID_TOOL_GROUPS.includes(group)) {
        errors.push(
          `Invalid tool group at index ${index}: "${group}". Valid groups are: ${VALID_TOOL_GROUPS.join(
            ', '
          )}`
        );
        isValidGroup = false;
      ...
      if (!VALID_TOOL_GROUPS.includes(groupName)) {
        errors.push(
          `Invalid tool group at index ${index}: "${groupName}". Valid groups are: ${VALID_TOOL_GROUPS.join(
            ', '
          )}`
        );
        isValidGroup = false;
      ...
      if (typeof restriction !== 'object') {
        ...
        if (!restriction.fileRegex) {
          errors.push(
            `Missing required "fileRegex" in group restriction at index ${index}`
          );
          isValidGroup = false;
          ...
          try {
            ...
            if (invalidEscaping) {
              errors.push(
                `Regex pattern at index ${index} has invalid escaping for JSON: "${regexStr}"`
              );
              warnings.push(
                `In JSON, backslashes must be doubled: "\\." not "\\." for a literal period`
              );
              isValidGroup = false;
              ...
              if (fixMode) {
                ...
                console.log(
                  `    Auto-fix: Fixed JSON escaping in regex pattern: "${fixedRegex}"`
                );
              }
            }
          } catch (error) {
            errors.push(
              `Invalid regex pattern at index ${index}: "${restriction.fileRegex}" - ${error.message}`
            );
            isValidGroup = false;
        ...
        if (!restriction.description) {
          errors.push(
            `Missing required "description" in group restriction at index ${index}`
          );
          isValidGroup = false;
      ...
      errors.push(
        `Invalid group format at index ${index}. Must be a string or an array with 2 elements`
      );
      isValidGroup = false;
      ...
      if (fixMode) {
        ...
        console.log(
          `    Auto-fix: Removed invalid group format at index ${index}`
        );
      }
    }
  });
  ...
  if (fixMode && fixedGroups.length === 0) {
    ...
    console.log(
      `    Auto-fix: Added default "read" group since no valid groups were found`
    );
  }
...
function main() {
  ...
  if (options.fix) {
    console.log(
      `Fix mode enabled. Output will be written to: ${options.outputPath}`
    );
  }
